...
typedef RT_MODEL_heli_q8_calibrate_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * heli_q8_calibrate(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(heli_q8_calibrate_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(heli_q8_calibrate_DW.StreamCall1_Stream);
  }
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: heli_q8_calibrate -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
