 ...
 * C source code generated on : Mon Aug 31 13:16:51 2020
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  /* Sum: '<Root>/Sum3' */
  heli_q8_B.Sum3 = currentTime + heli_q8_B.PulseGenerator;
   ...
   *  Sum: '<S5>/Sum1'
   */
  heli_q8_B.Gain_j = (((heli_q8_B.Sum3 + heli_q8_B.Gain) -
                       heli_q8_B.PitchCounttorad) * heli_q8_P.K_pp -
                      heli_q8_P.K_pd * heli_q8_B.PitchTransferFcn) *
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (heli_q8_B.RateTransitiony > heli_q8_P.DeadZoney_End) {
      currentTime = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_End;
    } else if (heli_q8_B.RateTransitiony >= heli_q8_P.DeadZoney_Start) {
      currentTime = 0.0;
    } else {
      currentTime = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S4>/Gain: y'
     */
    heli_q8_B.Joystick_gain_y = heli_q8_P.Gainy_Gain * currentTime *
     ...
     *  Sum: '<Root>/Sum2'
     */
    rtb_Sum_p = (heli_q8_P.Gain1_Gain * heli_q8_B.Joystick_gain_y +
                 heli_q8_P.Constant1_Value) - heli_q8_B.Sum;
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    rtb_TmpSignalConversionAtToFile[1] = heli_q8_B.Gain_j;
    rtb_TmpSignalConversionAtToFile[2] = heli_q8_B.Sum_m;
    ...
    rtb_TmpSignalConversionAtToFile[7] = heli_q8_B.Sum;
          ...
          if (fp != (NULL)) {
            ...
            if (fwrite(u, sizeof(real_T), 10, fp) != 10) {
              rtmSetErrorStatus(heli_q8_M,
                                "Error writing to MAT-file lambda_1.mat");
              return;
            }
            ...
            if (((++heli_q8_DW.ToFile_IWORK.Count)*10)+1 >= 100000000) {
              (void)fprintf(stdout,
                            ...
                            "    allowed in MAT-file lambda_1.mat.\n");
            }
          }
        }
      }
   ...
   *  Sum: '<S1>/Add'
   */
  currentTime = (heli_q8_B.Sum_m - heli_q8_B.Gain_j) * heli_q8_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  currentTime = (heli_q8_B.Sum_m + heli_q8_B.Gain_j) * heli_q8_P.Backgain_Gain;
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "lambda_1.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file lambda_1.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,10,0,"data")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file lambda_1.mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "lambda_1.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file lambda_1.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file lambda_1.mat");
        return;
      }
      ...
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for data to MAT-file lambda_1.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (890640467U);
  heli_q8_M->Sizes.checksums[1] = (2269962394U);
  heli_q8_M->Sizes.checksums[2] = (4171477616U);
  heli_q8_M->Sizes.checksums[3] = (781106406U);
